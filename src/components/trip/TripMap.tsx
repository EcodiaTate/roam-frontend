...
// We use filled paths (not strokes) for crisp rendering at small sizes on the map.
// Each path is designed to be recognizable at 24-36px rendered size.
...
const ICON_PATHS = {
  ...
  wrench: "M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6-1.4 1.4-1.6-1.6a1 1 0 0 0-1.4 0l-5 5A3.8 3.8 0 0 0 2 18.5 3.5 3.5 0 0 0 5.5 22a3.8 3.8 0 0 0 3.4-4.9l5-5a1 1 0 0 0 0-1.4l-1.6-1.6 1.4-1.4 1.6 1.6a1 1 0 0 0 1.4 0l2-2A5 5 0 0 0 22 5.5V3l-2 2h-2V3l-2 2a1 1 0 0 0 0 1.4Z",
  ...
  building: "M3 21V3a1 1 0 0 1 1-1h7a1 1 0 0 1 1 1v18M3 21h18M12 7h4a1 1 0 0 1 1 1v13M7 5h2M7 9h2M7 13h2M7 17h2M15 11h2M15 15h2",
  ...
  palette: "M12 2a10 10 0 0 0-1 20 2 2 0 0 0 2-2v-.5a2 2 0 0 1 2-2h1.5A2 2 0 0 0 18.5 15.5 10 10 0 0 0 12 2ZM8.5 8a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 6a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM16 9a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3Z",
  ...
  // ── Utilities ──
  town: "M3 21V8l4-4 4 4v13M11 21V12h5l4 4v5M7 9v.01M7 13v.01M7 17v.01M15 17v.01",
  ...
  siren: "M12 2v2M4.9 4.9l1.4 1.4M19.1 4.9l-1.4 1.4M2 12h2M20 12h2M7 17.5A7 7 0 0 1 12 5a7 7 0 0 1 5 12.5M7 17.5v2.5h10v-2.5",
  question: "M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20Zm0 14v.01M12 12a2 2 0 1 0-2-2",

  // ── Hazard overlay ──
  cyclone: "M12 12a3 3 0 1 0 0 .01M21 12c-1 5-6 8-9 8-5 0-9-4-9-9a9 9 0 0 1 15-6.7M3 12c1-5 6-8 9-8 5 0 9 4 9 9a9 9 0 0 1-15 6.7",
...
// ══════════════════════════════════════════════════════════════
// TripMap.tsx — CATEGORY_CONFIG replacement
// ══════════════════════════════════════════════════════════════
//
// In src/components/trip/TripMap.tsx, find the existing
// `const CATEGORY_CONFIG: Record<string, CatConfig> = { ... };`
// block and REPLACE IT ENTIRELY with this:
//
// ══════════════════════════════════════════════════════════════
...
function trafficPointsGeoJSON(overlay: TrafficOverlay | null) {
  ...
  return {
    ...
    features: overlay.items
      ...
      .map((ev) => ({
        type: "Feature",
        properties: { id: ev.id, type: ev.type ?? "unknown", severity: ev.severity ?? "unknown", headline: ev.headline, iconId: `roam-traffic-${ev.type ?? "unknown"}` },
...
function hazardPointsGeoJSON(overlay: HazardOverlay | null) {
  ...
  return {
    type: "FeatureCollection",
    features: overlay.items.map((ev) => {
      if (ev.geometry && ev.geometry.type === "Point") {
        return { type: "Feature", properties: { id: ev.id, kind: ev.kind ?? "unknown", severity: ev.severity ?? "unknown", title: ev.title, iconId: `roam-hazard-${ev.kind ?? "unknown"}` }, geometry: ev.geometry };
      }
      if (ev.bbox && ev.bbox.length === 4 && !ev.geometry) {
        return { type: "Feature", properties: { id: ev.id, kind: ev.kind ?? "unknown", severity: ev.severity ?? "unknown", title: ev.title, iconId: `roam-hazard-${ev.kind ?? "unknown"}` }, geometry: { type: "Point", coordinates: [(ev.bbox[0] + ev.bbox[2]) / 2, (ev.bbox[1] + ev.bbox[3]) / 2] } };
      }
      return null;
    }).filter(Boolean),
  } as any;
...
function userLocGeoJSON(pos: RoamPosition | null | undefined) {
  ...
  return {
    type: "FeatureCollection",
    features: [{ type: "Feature", properties: { accuracy: pos.accuracy, heading: pos.heading, speed: pos.speed }, geometry: { type: "Point", coordinates: [pos.lng, pos.lat] } }],
  } as any;
}
...
const TRAFFIC_SEV_COLORS: Record<string, string> = { major: "#ef4444", moderate: "#f59e0b", minor: "#3b82f6", info: "#64748b", unknown: "#64748b" };
...
export function TripMap(props: Props) {
    ...
    return `<div style="font-family:inherit;min-width:160px;max-width:260px">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">
        <div style="flex:1;min-width:0">
          ...
          <span style="display:inline-block;margin-top:3px;font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:0.5px;color:${sevColor};background:color-mix(in srgb, ${sevColor} 12%, transparent);padding:2px 7px;border-radius:5px;">${escapeHtml(severity)}</span>
        </div>
      </div>
  ...
  useEffect(() => {
    ...
    document.addEventListener("click", (e) => {
      ...
      if (placeId) { e.preventDefault(); e.stopPropagation(); onNavToGuideRef.current?.(placeId); }
    });

    map.on("style.load", async () => {
      await Promise.all([loadHeadingArrow(map), loadCategoryIcons(map), loadOverlayIcons(map), loadStopIcons(map)]);
      ...
      /* ════════════════════════════════════════════════════════════════
         ...
         7. Alert highlight (ring → ping animation)
         ════════════════════════════════════════════════════════════════ */
      ...
      if (!map.getLayer(TRAFFIC_POLY_LAYER)) {
        map.addLayer({
          ...
          paint: {
            "fill-color": ["match", ["get", "severity"], "major", "rgba(239,68,68,0.22)", "moderate", "rgba(245,158,11,0.18)", "minor", "rgba(59,130,246,0.12)", "rgba(100,116,139,0.08)"],
            "fill-opacity": 0.85,
          },
        });
      }
      ...
      if (!map.getLayer(TRAFFIC_LINE_CASING)) {
        map.addLayer({
          id: TRAFFIC_LINE_CASING, type: "line", source: TRAFFIC_LINE_SRC,
          layout: { "line-cap": "round", "line-join": "round" },
          paint: {
            "line-color": ["match", ["get", "severity"], "major", "rgba(153,27,27,0.5)", "moderate", "rgba(146,64,14,0.4)", "minor", "rgba(30,64,175,0.3)", "rgba(51,65,85,0.25)"],
      ...
      if (!map.getLayer(TRAFFIC_PULSE_LAYER)) {
        map.addLayer({
          ...
          paint: {
            ...
            "circle-color": ["match", ["get", "severity"], "major", "rgba(239,68,68,0.18)", "moderate", "rgba(245,158,11,0.14)", "rgba(100,116,139,0.08)"],
            "circle-stroke-color": ["match", ["get", "severity"], "major", "rgba(239,68,68,0.35)", "moderate", "rgba(245,158,11,0.28)", "rgba(100,116,139,0.18)"],
      ...
      if (!map.getLayer(HAZARD_POLY_LAYER)) {
        map.addLayer({
          ...
          paint: {
            "fill-color": ["match", ["get", "severity"], "high", "rgba(220,38,38,0.18)", "medium", "rgba(234,88,12,0.14)", "low", "rgba(37,99,235,0.10)", "rgba(100,116,139,0.06)"],
            "fill-opacity": 0.75,
          },
        });
      }
      if (!map.getLayer(HAZARD_POLY_OUTLINE)) {
        map.addLayer({
          ...
          paint: {
            "line-color": ["match", ["get", "severity"], "high", "rgba(220,38,38,0.6)", "medium", "rgba(234,88,12,0.5)", "low", "rgba(37,99,235,0.4)", "rgba(100,116,139,0.3)"],
            "line-width": 2.5, "line-dasharray": [3, 2], "line-opacity": 0.85,
          },
        });
      }
      ...
      if (!map.getLayer(SUG_CLUSTER_CIRCLE)) {
        map.addLayer({
          ...
          paint: {
            ...
            "circle-stroke-color": "rgba(255,255,255,0.3)", "circle-stroke-width": 2, "circle-opacity": 0.92,
          },
        });
      }
      ...
      if (!map.getLayer(SUG_ICON_LAYER)) {
        map.addLayer({
          ...
          filter: ["!", ["has", "point_count"]], minzoom: 10,
          layout: {
            "icon-image": ["get", "iconId"],
      ...
      if (!map.getLayer(SUG_LABEL_LAYER)) {
        map.addLayer({
          ...
          layout: {
            ...
            "text-offset": [0, 1.4], "text-anchor": "top", "text-max-width": 10,
      ...
      const handleSugClick = (e: any) => {
        ...
        try {
          popupRef.current?.remove();
          popupRef.current = new maplibregl.Popup({ closeButton: true, closeOnClick: true, className: "trip-map-popup", maxWidth: "280px" }).setLngLat(e.lngLat).setHTML(html).addTo(map);
        } catch {}
      };
      ...
      if (!map.getLayer(STOPS_SHADOW)) {
        map.addLayer({
          id: STOPS_SHADOW, type: "circle", source: STOPS_SRC, minzoom: 5,
          paint: {
      ...
      if (!map.getLayer(STOPS_OUTER)) {
        map.addLayer({
          id: STOPS_OUTER, type: "circle", source: STOPS_SRC, minzoom: 3,
          paint: {
      ...
      if (!map.getLayer(STOPS_INNER)) {
        map.addLayer({
          id: STOPS_INNER, type: "circle", source: STOPS_SRC, minzoom: 3,
          paint: {
      ...
      if (!map.getLayer(STOP_PULSE)) {
        map.addLayer({
          id: STOP_PULSE, type: "circle", source: STOPS_SRC, minzoom: 8,
      ...
      if (!map.getLayer(STOP_ICON_LAYER)) {
        map.addLayer({
          id: STOP_ICON_LAYER, type: "symbol", source: STOPS_SRC, minzoom: 11,
          layout: {
            "icon-image": ["get", "iconId"],
      ...
      if (!map.getLayer(STOP_LABELS)) {
        map.addLayer({
          id: STOP_LABELS, type: "symbol", source: STOPS_SRC, minzoom: 9,
          layout: {
            "text-field": ["get", "name"], "text-font": ["Noto Sans Bold"],
            "text-size": ["interpolate", ["linear"], ["zoom"], 9, 10, 14, 13, 16, 15],
            "text-offset": [0, 1.6], "text-anchor": "top", "text-max-width": 8,
            "text-optional": true, "text-allow-overlap": false,
          },
      ...
      if (!map.getLayer(STOP_FOCUS_RING)) {
        map.addLayer({
          id: STOP_FOCUS_RING, type: "circle", source: STOPS_SRC,
      ...
      if (!map.getLayer(USER_LOC_ACCURACY)) {
        map.addLayer({
          ...
          paint: {
            "circle-radius": 30,
            "circle-color": "rgba(37,99,235,0.07)", "circle-stroke-color": "rgba(37,99,235,0.22)",
            "circle-stroke-width": 1.5, "circle-opacity": 1,
          },
        });
      }
  ...
  useEffect(() => {
      ...
      try {
        ...
        for (const f of feats) {
          if (f?.properties?.id === id && f?.geometry?.type === "Point") {
            coord = f.geometry.coordinates as [number, number]; break;
          }
        }
      } catch {}
      if (coord) break;
    ...
    if (!map.getLayer(ALERT_HIGHLIGHT_PING)) {
      map.addLayer({
        id: ALERT_HIGHLIGHT_PING, type: "circle", source: ALERT_HIGHLIGHT_SRC,
    ...
    if (!map.getLayer(ALERT_HIGHLIGHT_RING)) {
      map.addLayer({
        id: ALERT_HIGHLIGHT_RING, type: "circle", source: ALERT_HIGHLIGHT_SRC,
